<template>
    <div class="note-class-wrapper">
        <div class="note-class" :class="{ hide: showMenu }">
            <header>
                <div @click="show"><van-icon name="wap-nav" /></div>
                <div>
            ...
            <section>
                <div class="note-item" v-for="(item, index) in noteClassList" :key="index"
                    :style="{ backgroundColor: item.bgColor }" @click="gotoList(item.title)">
                    <span class="title">{{ item.title }}</span>
                </div>
            </section>
        </div>
        <!-- menu -->
        <Menu class="menu" :class="{ show: showMenu }" @changeshow="show" />
    </div>
</template>
